...
sys_exofork(void)
{
	...
	struct Env * e;
	int result;
	if ((result = env_alloc(&e, sys_getenvid())) < 0)
        return result;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	pte_t *pte;
	struct PageInfo * pp;
	if(envid2env(envid, &des, 0)<0){
		return -E_BAD_ENV;
	}
	if(des->env_ipc_recving!=true){
	...
	if(srcva<(void *)UTOP){
		...
		if(des->env_ipc_dstva!=0){
			if(page_insert(des->env_pgdir,pp,des->env_ipc_dstva,perm)<0){
				return -E_NO_MEM;
			}
		}
		des->env_ipc_perm = perm;
	}
	des->env_ipc_recving = false;
    des->env_ipc_from = curenv->env_id;
	des->env_ipc_value = value;
	des->env_status = ENV_RUNNABLE;
	des->env_tf.tf_regs.reg_eax = 0;
	return 0;
}
...
// The syscall of transmit
// Return the transmit length.
static int
...
// The syscall of receive
// Return the receive length.
static int
sys_e1000_receive(void *buf)
{
	return e1000_receive_data(buf);
}
